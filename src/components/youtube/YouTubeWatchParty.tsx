...
import { X, Users, Send, Copy, Check, UserPlus, MonitorUp, MonitorOff, Loader2, PlayCircle } from 'lucide-react';
...
interface YouTubeWatchPartyProps {
  onClose: () => void;
  videoId?: string;
  videoTitle?: string;
}

export function YouTubeWatchParty({ onClose, videoId, videoTitle }: YouTubeWatchPartyProps) {
  const { user } = useAuth();
  ...
  const partyId = useRef(`yt-party-${Date.now()}`);
  ...
  const [syncedVideoId, setSyncedVideoId] = useState<string | null>(videoId || null);
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`youtube-watch-party-${partyId.current}`);
    ...
    channel
      ...
      .on('broadcast', { event: 'video-sync' }, (payload) => {
        if (payload.payload.senderId !== user.id) {
          setSyncedVideoId(payload.payload.videoId);
        }
      })
  ...
  const startParty = async () => {
    ...
    for (const friend of selectedFriends) {
      await supabase.from('direct_messages').insert({
        ...
        message: `🎬 ${user!.username} invited you to a YouTube Watch Party!${videoTitle ? ` Watching: ${videoTitle}` : ''} Join them now!`,
      });
    }
  ...
  return (
    <div className="fixed inset-0 z-[150] bg-background/95 backdrop-blur-lg flex items-center justify-center p-4">
      <div className="w-full max-w-5xl bg-card border border-border/50 rounded-2xl overflow-hidden shadow-2xl max-h-[90vh] flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-border/30 flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-foreground">YouTube Watch Party</h2>
        ...
        {!partyStarted ? (
          ...
          <div className="p-4 overflow-y-auto">
            {videoTitle && (
              <div className="mb-4 p-3 bg-red-500/10 border border-red-500/20 rounded-lg">
                ...
                <p className="text-foreground font-medium truncate">{videoTitle}</p>
              </div>
            )}
            ...
            <button
              ...
              className="w-full mt-4 py-3 bg-red-500 hover:bg-red-600 text-white font-medium rounded-lg transition-all disabled:opacity-50 flex items-center justify-center gap-2"
            >
          ...
          <div className="flex flex-col lg:flex-row flex-1 overflow-hidden">
            ...
            <div className="flex-1 bg-black/50 flex flex-col min-h-[300px] lg:min-h-0">
              {/* Screen Display */}
              <div className="flex-1 relative flex items-center justify-center">
                ...
                ) : syncedVideoId && !isHost ? (
                  <div className="w-full h-full">
                    <iframe
                      src={`https://www.youtube.com/embed/${syncedVideoId}?autoplay=1&rel=0&modestbranding=1`}
            ...
            {/* Chat Section */}
            <div className="w-full lg:w-80 border-t lg:border-t-0 lg:border-l border-border/30 flex flex-col max-h-[300px] lg:max-h-none">
              ...
              <form onSubmit={handleSendMessage} className="p-3 border-t border-border/30 flex-shrink-0">
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-muted/30 border border-border/30 rounded-lg px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-red-500/50"
                    disabled={isSending}
                  />
                  <button
                    type="submit"
